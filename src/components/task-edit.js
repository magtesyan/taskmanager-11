...
const createTaskEditTemplate = (task, options = {}) => {
  const {repeatClass, deadlineClass, isDateShowing, date, time, isRepeatingTask, color, activeRepeatingDays, currentDescription, externalData} = calcTaskDetails(task, options);
  const description = encode(currentDescription);

  const colorsMarkup = createColorsMarkup(COLORS, color);
  const repeatingDaysMarkup = createRepeatingDaysMarkup(DAYS, activeRepeatingDays);

  const isBlockSaveButton = (isDateShowing && isRepeatingTask) || !isAllowableDescriptionLength(description);
